import React from "react";

const TLShoppingBag = ({
  className,
  size = "16",
}: {
  className?: string;
  size?: string;
}) => {
  return (
    <svg
      className={className}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        opacity="0.4"
        d="M4.38809 9.26285C4.74282 7.37096 6.39472 6 8.31958 6H15.6802C17.605 6 19.2569 7.37095 19.6117 9.26285L21.1117 17.2628C21.5732 19.7245 19.6847 22 17.1802 22H6.81958C4.31502 22 2.42653 19.7245 2.88809 17.2628L4.38809 9.26285Z"
        fill="currentColor"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M8.25 5C8.25 2.92893 9.92893 1.25 12 1.25C14.0711 1.25 15.75 2.92893 15.75 5V8C15.75 8.41421 15.4142 8.75 15 8.75C14.5858 8.75 14.25 8.41421 14.25 8V5C14.25 3.75736 13.2426 2.75 12 2.75C10.7574 2.75 9.75 3.75736 9.75 5V8C9.75 8.41421 9.41421 8.75 9 8.75C8.58579 8.75 8.25 8.41421 8.25 8V5Z"
        fill="currentColor"
      />
    </svg>
  );
};

export default TLShoppingBag;
