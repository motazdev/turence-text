...
const VerifyEmailChangeDialog = ({
  isOpen,
  setIsOpen,
  setIsOpenChangeEmail,
}: {
  isOpen: boolean;
  setIsOpenChangeEmail: Dispatch<SetStateAction<boolean>>;
  ...
  const resend = async () => {
    try {
      ...
      const data = await authService.emailSendVerifyCode();
      if (data.status) {
  ...
  const { mutate: sendCode } = useMutation({
    ...
    mutationFn: () => authService.emailSendVerifyCode(),
    onError: (error) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: { code: string }) =>
      authService.emailCheckVerifyCode({ code: data.code }),
...
export default VerifyEmailChangeDialog;
